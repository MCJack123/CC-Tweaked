...
local function mk_tbl(str, name)
    ...
    return setmetatable({}, { __newindex = function() error(msg, 2) end
...
textutils.empty_json_array = mk_tbl("[]", "empty_json_array")
...
-- @see textutils.serialiseJSON
-- @see textutils.unserialiseJSON
textutils.json_null = mk_tbl("null", "json_null")
...
do
    ...
    serializeJSONString = function(s, options)
        if  not (options and options.unicode_strings and s:find("[\x80-\xff]")) then
            return '"' .. s:gsub("[\0-\x1f\"\\]", map):gsub("[\x7f-\xff]", hexify) .. '"'
        end
        local retval = '"'
        for _, code in utf8.codes(s) do
            if  (code > 0xFFFF) then
                ...
                code = code - 0x10000
                local high = bit32.extract(code, 10, 10) + 0xD800
                local low  = bit32.extract(code,  0, 10) + 0xDC00
...
local function serializeJSONImpl(t, tracking, options)
    local sType = type(t)
    if  (t == textutils.empty_json_array) then
        return "[]"
    elseif(t == textutils.json_null) then
        return "null"
    ...
    for k, v in pairs(t) do
        ...
        elseif((type(k) == "number") and (k > largestArrayIndex)) then
            -- the largest index is kept to avoid losing half the array
            -- if there is any single nil in that array
            largestArrayIndex = k
        end
    end
    for k = 1, largestArrayIndex, 1 do
        -- the array is read up to the very last valid array index
        -- ipairs() would stop at the first nil value and we would lose any data after.
        local sEntry
        if  (t[k] == nil) then
...
do
    ...
    local function decode_impl(str, pos, opts)
        ...
        elseif(c == "[") then
            ...
            return arr, pos + 1
        elseif(c == "") then
            error_at(pos, 'Unexpected end of input.')
        end
...
function textutils.serializeJSON(t, options)
...
function textutils.complete(sSearchText, tSearchTable)
    ...
    while tTable do
        for k, v in pairs(tTable) do
            if  (   not tSeen[k]
                and (type(k) == "string")
                and (string.find(k, sPart, 1, true) == 1)
                and not g_tLuaKeywords[k]
                and string.match(k, "^[%a_][%a%d_]*$")
                ) then
